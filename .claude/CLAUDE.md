...
- **Cutting a release ("facciamo la release X.Y.Z")**: Claude's job stops at bumping the version (`engine/Cargo.toml`) and `CHANGELOG.md`, committing and pushing that to the working branch. Merging the PR into `main` and pushing the `EuLLM-v*` tag are done by the user, not Claude — do not merge the PR and do not attempt `git push` of a tag. Asked repeatedly; stop re-litigating it.
