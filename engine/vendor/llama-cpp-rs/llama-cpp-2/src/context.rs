...
impl<'model> LlamaContext<'model> {
    ...
    /// `llama_rs_memory_breakdown_print` (`common_memory_breakdown_print`
    /// underneath, the exact function those binaries call), so a comparison
    /// against them is apples-to-apples instead of a guess from two
    /// different processes' free-VRAM numbers.
    ///
    /// The table is emitted through `common_log`, a separate logging
    /// system from the raw `llama_log_set` callback eullm silences via
    /// `LlamaBackend::void_logs` — a first version of this method tried to
    /// route around `void_logs` and did nothing, because that was never
    /// the mechanism actually swallowing the table. The real gate
    /// (`common_log`'s own verbosity threshold, which drops this table by
    /// default) is handled on the C++ side, in the `llama_rs_memory_
    /// breakdown_print` wrapper itself — see the comment there.
    #[cfg(feature = "common")]
